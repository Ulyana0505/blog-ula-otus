...
function autoInit() {
    ...
    for (const el of els) {
        ...
        for (const elItem of [...el.querySelectorAll("." + carouselItemCss)]) {
            ...
            if (img) {
                slides.push({src: img.getAttribute("src"), text: text?.innerHTML.trim() ?? ""});
            }
        }
        ...
        initialParams.timeView = Number(el.dataset.timeView) > 0 ? Number(el.dataset.timeView) : initialParams.timeView;
        initialParams.width = Number(el.dataset.width) > 0 ? Number(el.dataset.width) : initialParams.width;
        initialParams.height = Number(el.dataset.height) > 0 ? Number(el.dataset.height) : initialParams.height;
        initialParams.timeSliding = Number(el.dataset.timeSliding) > 0 ? Number(el.dataset.timeSliding) : initialParams.timeSliding;
...
export function initCarousel(parentElem, slideRows = [], initialParams = {}) {
    ...
    const carouselParams = startCarousel(parentElem, slideRows.length, initialParams);
    ...
    return {
        ...
        appendSlide(src, text) {
            ...
            if (els.length === 1) {
                ...
                carouselParams.all = parentElem.getElementsByClassName(carouselItemCss).length;
        ...
        updateSlide(index, src, text) {
            ...
            if (els.item(index)) {
                els.item(index).innerHTML = addItem({src, text}, initialParams);
            }
        }
    }
}
...
function addItem(row, initialParams) {
    return `<div class="${carouselSlideCss}"><img src="${row.src}" width="${initialParams.width}" height="${initialParams.height}" alt="" />${row.text ? `<div class="${carouselTextCss}">${row.text}</div>` : ""}</div>`;
}
...
function updateIndicators(carouselParams) {
    const elsIndicators = carouselParams.parentElem.getElementsByClassName(carouselIndicatorsCss);
    if (elsIndicators.length === 1) {
        elsIndicators.item(0).innerHTML = Array.from({length: carouselParams.all})
            .map((_, index) => `<button data-index="${index}"></button>`).join("");
    }
}
...
function initCarouselView(parentElem, slideRows, initialParams) {
    const items = slideRows.map(row => `<div class="${carouselItemCss}">${addItem(row, initialParams)}</div>`)
    ...
    parentElem.setAttribute("style", `--sliding-time: ${initialParams.timeSliding}ms;`);
    parentElem.classList.add(carouselCss);
    parentElem.style.width = initialParams.width + "px";
    parentElem.innerHTML = html.join("");
    parentElem.getElementsByClassName(carouselItemCss)[0].classList.add(activeCss);
...
function startCarousel(parentElem, all, initialParams) {
    ...
    if (initialParams.withControl) {
        parentElem.getElementsByClassName(carouselControlPrev)[0].addEventListener("click", () => handleNextStep(carouselParams, directionOptions.prev));
        parentElem.getElementsByClassName(carouselControlNext)[0].addEventListener("click", () => handleNextStep(carouselParams, directionOptions.next));
    }
    if (initialParams.withIndicators) {
        updateIndicators(carouselParams);
        carouselParams.parentElem.getElementsByClassName(carouselIndicatorsCss).item(0).addEventListener("click", (e) => handleIndicator(carouselParams, e.target));
    }
...
function nextTimer(carouselParams) {
    ...
    carouselParams.timerNext = setTimeout(handleNextByTimer, carouselParams.initial.timeView, carouselParams);
}
...
function startSliding(carouselParams) {
    ...
    let nextIndex = carouselParams.nextByControl > byControlDefault ? carouselParams.nextByControl : (carouselParams.currentIndex + carouselParams.nextByDirection);
     ...
     */
    const items = [...carouselParams.parentElem.querySelectorAll(`.${carouselItemCss}`)];
    ...
    function transitionEnd() {
        ...
        if (carouselParams.nextByControl !== byControlDefault || carouselParams.nextByDirection !== directionOptions.none) {
            ...
            carouselParams.timerNext = setTimeout(startSliding, 300, carouselParams);
        } else {
